...
struct WebUIIntroduction: HTML {
  ...
  static var document: Document {
    let title = "Introduction to WebUI"
    let slug = title.slugified()
    let description =
      "Learn about WebUI, a Swift-based framework for building fast and secure web applications with a SwiftUI-inspired syntax."
    let date = ISO8601DateFormatter().date(from: "2025-04-01T09:00:00Z") ?? Date()
