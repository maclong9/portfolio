...
struct BuildingModernWebTools: Document {
    var path: String? { "posts/building-modern-web-tools" }

    var metadata: Metadata {
        Metadata(from: Application().metadata, title: "Building Modern Web Tools - Mac Long")
    }

    private var article: ArticleResponse? {
        do {
            let articles = try ArticlesService.fetchAllArticles()
            return articles.first { $0.slug == "building-modern-web-tools" }
        } catch {
    ...
    var body: some Markup {
        guard let article = article else {
            return AnyMarkup(
                Layout(
                    path: "posts/building-modern-web-tools",
