...
struct WritingACommandLineToolInSwift: Document {
    var path: String? { "posts/writing-a-command-line-tool-in-swift" }

    var metadata: Metadata {
        Metadata(from: Application().metadata, title: "Writing a Command Line Tool in Swift - Mac Long")
    }

    private var article: ArticleResponse? {
        do {
            let articles = try ArticlesService.fetchAllArticles()
            return articles.first { $0.slug == "writing-a-command-line-tool-in-swift" }
        } catch {
    ...
    var body: some Markup {
        guard let article = article else {
            return AnyMarkup(
                Layout(
                    path: "posts/writing-a-command-line-tool-in-swift",
