...
struct MyPersonalSetup: Document {
    var path: String? { "posts/my-personal-setup" }

    var metadata: Metadata {
        Metadata(from: Application().metadata, title: "My Development Environment Setup - Mac Long")
    }

    private var article: ArticleResponse? {
        do {
            let articles = try ArticlesService.fetchAllArticles()
            return articles.first { $0.slug == "my-personal-setup" }
        } catch {
