...
public struct Layout: Element {
    ...
    public var body: some Markup {
        BodyWrapper(classes: [
            ...
            Stack(classes: [
                "min-h-screen", "flex", "flex-col", "bg-zinc-50", "dark:bg-zinc-900", "transition-colors",
                "duration-300",
            ]) {
                Header(classes: [
                    "fixed", "top-0", "left-0", "right-0", "bg-white", "dark:bg-zinc-800", "border-b",
                    "border-zinc-200", "dark:border-zinc-700", "px-4", "py-6", "z-50",
                    "transition-transform", "duration-300", "ease-in-out",
                ]) {
                    Stack(classes: ["max-w-4xl", "mx-auto", "flex", "items-center", "justify-between", "w-full"]) {
                        Stack(classes: ["text-zinc-900", "dark:text-zinc-100", "font-medium"]) {
                            Text("Mac Long")
                        }
                        NavigationLinks()
                        // Theme Dropdown
