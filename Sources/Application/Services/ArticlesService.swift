...
public struct ArticleResponse: Identifiable {
  ...
  ) -> (title: String?, keywords: [String], publishedDate: Date?, summary: String?, cleanedContent: String) {
    var title: String?
    var keywords: [String] = []
      ...
      if let summaryMatch = remainingContent.range(of: summaryStrongPattern, options: .regularExpression) {
        ...
        let absoluteRange =
          cleanedContent.index(
            cleanedContent.startIndex,
            offsetBy: absoluteStart
          )..<cleanedContent.index(cleanedContent.startIndex, offsetBy: absoluteEnd)
